...
export const createLayout = catchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (type === "banner") {
            ...
            // Upload to Cloudinary ONLY if image is provided as a non-empty string
            ...
            const { faq } = req.body;
            await LayoutModel.create({ type: "faq", faq: faq || [] });
        } else if (type === "categories") {
            const { categories } = req.body;
            await LayoutModel.create({ type: "categories", categories: categories || [] });
        } else {
        ...
        return next(new ErrorHandler(error.message || "Failed to create layout", 500));
    }
});
